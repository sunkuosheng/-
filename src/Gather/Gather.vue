...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            //分页查询数据
            async queryInfoForPage(fid, page, rows) {
                try {
                    let result = await queryInfoForPage({fid: fid, page: page, rows: rows}, "GET");
                    if (result.code == 0) {
                        console.log(result);
...
//                        this.total = result.data.count;
        ...
        //在渲染成html后调用 获取字典数据
        mounted() {
            this.queryInfoForPage(this.fid, this.currentPage, this.pagesize);
        },
    }
</script>
<style>
